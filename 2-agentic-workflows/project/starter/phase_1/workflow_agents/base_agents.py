...
class DirectPromptAgent:
    """
    A simple agent that directly prompts the LLM without any additional context.
    It passes the user's prompt directly to the model and returns the response.
    """
...
class AugmentedPromptAgent:
    """
    An augmented agent that prompts the LLM to respond with a specified persona.
    The persona is set via a system role. This influences how the agent responds.
    """
...
class EvaluationAgent:
    """
    An agent that evaluates the responses of a worker agent against criteria specified.
    It iteratively refines the worker agent's response until it meets the criteria or max iterations is reached.
    """
    ...
    def evaluate(self, initial_prompt):
        ...
        # Iteratively gets a response from the worker agent, evaluates it, and refines if needed
        client = OpenAI(base_url="https://openai.vocareum.com/v1", api_key=self.openai_api_key)
        ...
        for i in range(self.max_interactions): # TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            print(f"Prompt:\n{prompt_to_evaluate}")
            response_from_worker = self.agent_to_evaluate.respond(prompt_to_evaluate) # TODO: 3 - Obtain a response from the worker agent
...
class RoutingAgent():
    """
    An agent that routes user prompts to a specialized agent
    ...
    def route(self, user_input):
        """
        Route the user input to the most appropriate agent based on semantic similarity.

        Parameters:
        user_input (str): The user's prompt to be routed.
...
class ActionPlanningAgent:
    """
    Agent extracts actionable steps from a user prompt using the provided knowledge.
    It breaks down high-level tasks into specific, executable, actionable steps.
    """
